...
We use createCohortInfo() to make CohortInfo class objects, then assign the CohortInfo objects to the variable covCohorts as a list. Then we set the covariateCohorts parameter of createCohortLineItemBatch() equal to our list of CohortInfo objects (covCohorts). In the createCohortlineItem() or CreateCohortLineItemBatch() calls we must specify into which table these cohorts are instantiated.
